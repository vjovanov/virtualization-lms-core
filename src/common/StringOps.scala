...
trait StringOps extends Variables with OverloadHack {
  ...
  def infix_endsWith(s: Rep[String], ews: Rep[String]) = string_endswith(s, ews)
  ...
  def string_endswith(s: Rep[String], ews: Rep[String]): Rep[Boolean]
}
...
trait StringOpsExp extends StringOps with VariablesExp {
  ...
  def string_endswith(s: Exp[String], ews: Exp[String]) = StringEndsWith(s,ews)
