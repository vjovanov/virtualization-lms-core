...
trait MiscOpsExp extends MiscOps with ObjectOpsExp with EffectExp {
  ...
  def print(x: Exp[Any])(implicit pos: SourceContext) = reflectEffect(Print(x.toStringL)) // TODO: simple effect
  def println(x: Exp[Any])(implicit pos: SourceContext) = reflectEffect(PrintLn(x.toStringL)) // TODO: simple effect
