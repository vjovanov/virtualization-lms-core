...
class TestStruct extends FileDiffSuite {
  ...
  trait Impl extends DSL with ComplexStructExp with ArrayLoopsExp with ArithExp with OrderingOpsExp with VariablesExp 
    ...
    val codegen = new ScalaGenStruct with ScalaGenArrayLoops with ScalaGenArith with ScalaGenOrderingOps 
    ...
    withOutFile(prefix+"struct2") {
      trait Prog extends DSL {
        def test(x: Rep[Int]) = {
          ...
          // don't want them to be reordered. plus: will reflect/reify prevent matchting on struct
