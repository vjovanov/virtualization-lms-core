...
trait LMSTransform extends LoweringTransform with ObjectOpsExp {

  // list of all transformers to be applied
  private var _transformers: List[WorklistTransformer] = Nil

  /*
   * return the set of transformers to be applied
   */
  def transformers: List[WorklistTransformer] = _transformers

  /*
   * Very very basic API for registering new transformers. We will elaborate on this when cost models come.
   */
  def prependTransformer(t: WorklistTransformer): Unit = {
    _transformers ::= t
  }

  def appendTransformer(t: WorklistTransformer): Unit = {
    _transformers :+= t
  }
